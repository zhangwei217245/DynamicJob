...
import x.spirit.dynamicjob.mockingjay.hbase.HBaseWriteMethods._
import x.spirit.dynamicjob.mockingjay.hbase.HBaseUtils.createJob
...
sealed abstract class HBaseWriteHelpers[A] {
  protected def convert(id: String, values: Map[String, Map[String, A]], put: PutAdder[A]) = {
    ...
    for {
      ...
      empty = false
      put(p, Bytes.toBytes(family), Bytes.toBytes(key), value)
    }
