...
import x.spirit.dynamicjob.mockingjay.hbase.HFileMethods._
  ...
  def toHBaseBulk(tableNameStr: String, family: String, numFilesPerRegionPerFamily: Int = 1)(implicit config: HBaseConfig, ord: Ordering[C]) = {
    ...
    val rdd = mapRdd.flatMap {
      case (k, m) =>
        ...
        m map { case (h, v) => ck((keyBytes, Bytes.toBytes(h)), v) }
    }

    saveAsHFile(getPartitionedRdd(rdd, kvf(Bytes.toBytes(family)), partitioner), table, regionLocator, connection)
  }
}
  ...
  def toHBaseBulk(tableNameStr: String, numFilesPerRegionPerFamily: Int = 1)(implicit config: HBaseConfig, ord: Ordering[C]) = {
    ...
    val rdds = for {
      ...
      rdd = mapRdd
        ...
        .flatMap {
          case (k, m) =>
            m map { case (h, v) => ck((k, Bytes.toBytes(h)), v) }
        }
    } yield getPartitionedRdd(rdd, kv(Bytes.toBytes(f)), partitioner)
